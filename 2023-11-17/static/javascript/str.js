// 문자열
// 자바스크립트에서 문자열은 String 객체 라는 것으로 저장이 되고
// 사용된다. String 객체에는 다양한 문자열을 처리하기 위한 메소드가 있다.
...
// 3. 문자열에서 특정문자 또는 문자열이 있나? 어디에 있나? - indexOf()
...
// 4.indexOf는 문자열의 첫번째부터 검색을 하기 대문에
// 뒷쪽에 있는 문자나 문자열이 결과로 나오지 않을 수 있다.
// lastIndexOf() - 문자열의 뒷쪽에서 검색
...
console.log( test.match(regex) ); //.test() - match() - 패턴에 매칭되는 문자열들을 가지고온다.

// i - ignore case 대소문자 구별없이 검색
// g - global 문자열 모든 내 패턴 검색
...
// 7. 문자열 추출 - substring()
console. log( word.substring(0,7) ); //0번째부터 7번째 이전까지 추출
...
console.log( text.match(/^남기현\s/g));
...
var money = member[1].split(" ")[1].replace(/[^0-9]/,'');
console.log(money);
...
var 송재영 = member[3].split(" ")[1].replace(/./, "434,329원");
console.log(송재영);
...
console.log("공백제거 전: " +word);
console.log("공백제거 후: " +word.trim());

// 11. 문자열 반복 - repeat()
word = "apple";
console.log( word.repeat(3) );

// 12. 문자열 위치 정렬 도는 채우기 - padStart(), padEnd()
...
/*
    ...
    .replace(원본문자열, 변환문자열) - 문자열에서 특정 문자 또는 문자열을 지정한 문자 또는 문자열로
    ...
    .slice(시작번호, 끝번호), .slice(시작번호, 끝번호)
            - 문자열에서 지정된 위치의 문자열을 추출, 음수 사용시 뒤에서 부터 추출
